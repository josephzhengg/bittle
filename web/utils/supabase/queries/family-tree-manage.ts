...
export const createPointSubmission = async (
  ...
  const { error: insertError } = await supabase
    .from('point_submissions')
    .insert({ connection_id, prompt, point })
    .single();
...
export const createPointSubmissionWithChallenge = async (
  ...
  if (challengeError || !challenge) {
    throw new Error(`Error fetching challenge: ${challengeError?.message}`);
  }
  ...
  const { error: updateError } = await supabase
    .from('connections')
    .update({ points: (connection.points || 0) + (challenge.point_value || 0) })
    .eq('id', connection_id);
  if (updateError) {
    throw new Error(`Error updating connection points: ${updateError.message}`);
  }
